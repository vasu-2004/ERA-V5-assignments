...
// download the embedded tokenizer.json. Inside the claude.ai artifact sandbox
// use the runtime downloads capability; on a plain host (Netlify/local) fall
// back to a same-origin blob download.
function blobFallback(json,note){
...
$('dl-btn').addEventListener('click',function(){
  ...
  if(window.claude&&window.claude.downloads&&typeof window.claude.downloads.save==='function'){
    window.claude.downloads.save({filename:'tokenizer.json',data:json})
      .then(function(){note.textContent='Saved tokenizer.json ✓';})
      .catch(function(err){var c=err&&err.code;
        if(c==='declined'){note.textContent='Download cancelled — click the button again to retry.';}
        else{blobFallback(json,note);}});
  } else { blobFallback(json,note); }
});
// theme
